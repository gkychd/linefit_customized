...
class GroundSegmentation {
  ...
  PointType minZPointTo3d(const Bin::MinZPoint& min_z_point, const double& angle);
...
public:
  ...
  //GroundSegmentation(const GroundSegmentationParams& params);

  void segment(const PointCloud& cloud, std::vector<int>* segmentation, double& time);
...
void GroundSegmentation::segment(const PointCloud& cloud, std::vector<int>* segmentation, double& time) {
...
void GroundSegmentation::getMinZPointCloud(PointCloud* cloud) {
  ...
  for (auto seg_iter = segments_.begin(); seg_iter != segments_.end(); ++seg_iter) {
    for (auto bin_iter = seg_iter->begin(); bin_iter != seg_iter->end(); ++bin_iter) {
      const PointType min = minZPointTo3d(bin_iter->getMinZPoint(), angle);
      cloud->push_back(min);
    }
