...
inline int NUM_ZEROS = 5;

using namespace std;

inline double VEGETATION_THR = - SENSOR_HEIGHT * 3 / 4;
...
inline std::vector<int> outlier_classes = {UNLABELED, OUTLIER};
inline std::vector<int> ground_classes = {ROAD, PARKING, SIDEWALKR, OTHER_GROUND, LANE_MARKING, VEGETATION, TERRAIN};
inline std::vector<int> ground_classes_except_terrain = {ROAD, PARKING, SIDEWALKR, OTHER_GROUND, LANE_MARKING};

inline int count_num_ground(const pcl::PointCloud<PointXYZILID>& pc){
  int num_ground = 0;
...
inline std::map<int, int> set_initial_gt_counts(std::vector<int>& gt_classes){
...
inline std::map<int, int> count_num_each_class(const pcl::PointCloud<PointXYZILID>& pc){
  int num_ground = 0;
...
inline int count_num_outliers(const pcl::PointCloud<PointXYZILID>& pc){
  int num_outliers = 0;
...
inline void discern_ground(const pcl::PointCloud<PointXYZILID>& src, pcl::PointCloud<PointXYZILID>& ground, pcl::PointCloud<PointXYZILID>& non_ground){
  ground.clear();
  non_ground.clear();
  std::vector<int>::iterator iter;
...
inline void calculate_precision_recall(const pcl::PointCloud<PointXYZILID>& pc_curr,
...
inline int save_all_labels(const pcl::PointCloud<PointXYZILID>& pc, string ABS_DIR, string seq, int count){
...
inline void save_all_accuracy(const pcl::PointCloud<PointXYZILID>& pc_curr,
...
inline void pc2pcdfile(const pcl::PointCloud<PointXYZILID>& TP, const pcl::PointCloud<PointXYZILID>& FP,
